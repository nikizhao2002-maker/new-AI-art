...
function updateLanternDanceAnimation(dt, time) {
    ...
    lanternDanceInstances.forEach((inst) => {
        ...
        // 巡游不发光（按需求）：光晕隐藏、emissive 归零，鱼灯仅靠夜景灯光照亮
        if (inst.halo) inst.halo.visible = false;
        inst.emissiveMats.forEach((mat) => { mat.emissiveIntensity = 0; });
        // 纱绸泳动：每盏灯相位错开 + 转弯 C 弯
...
function animate() {
    ...
    if (lanternMeshGroup?.visible) {
        ...
        // 巡游阶段（LANTERN_SWARM）不发光；领灯展示阶段保留点亮仪式
        const paradeNoGlow = currentPhase === PHASES.LANTERN_SWARM;
        const emissiveMats = lanternMeshGroup.userData._emissiveMats;
        if (emissiveMats) {
            emissiveMats.forEach((mat) => {
                if (paradeNoGlow) { mat.emissiveIntensity = 0; return; }
        ...
        if (lanternPointLight) {
            lanternPointLight.intensity = paradeNoGlow ? 0 : Math.max(0.5, glow * 1.6) * flicker;
        }
        // 单盏鱼灯的纱绸泳动 + 灯影流彩
        ...
        // 个人光晕跟随 + 同律闪烁 + 随灯体缩放；巡游阶段隐藏
        if (personalLanternHalo) {
            if (paradeNoGlow) {
                personalLanternHalo.visible = false;
            } else if (personalLanternHalo.visible) {
