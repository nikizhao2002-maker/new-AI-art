...
let lanternParticleDissolveStarted = false;
let lanternParticleDissolveElapsed = 0;
let lanternParticleGestureFrames = 0;
...
function clearLanternDanceTimers() {
    ...
    lanternParticleDissolveStarted = false;
    lanternParticleDissolveElapsed = 0;
    lanternParticleGestureFrames = 0;
}
...
async function spawnLanternDanceGroup() {
    ...
    layouts.forEach((layout, index) => {
        ...
        const basePosition = new THREE.Vector3(layout[0], layout[1] + danceYOffset, layout[2]);
        mesh.visible = true;
...
async function enterLanternDance() {
    ...
    if (lanternMeshGroup) {
        lanternMeshGroup.visible = true;
        lanternMeshGroup.position.set(0, 18, 26);
...
function updateLanternDanceAnimation(dt, time) {
    ...
    if (!lanternDanceReleased) {
        ...
        lanternMeshGroup.position.y = 18 + Math.sin(time * 0.8) * 2.4;
    ...
    lanternMeshGroup.position.lerpVectors(new THREE.Vector3(0, 18, 26), new THREE.Vector3(18, 82, -10), eased);
...
async function showLanternReceivePreview() {
    ...
    await activateParticleScene({ handDrivesDissolve: false });
...
function startLanternParticleDissolve() {
    if (lanternSummonStage !== 'particles' || lanternParticleDissolveStarted) return;
    lanternParticleDissolveStarted = true;
    lanternParticleDissolveElapsed = 0;
    lanternParticleGestureFrames = 0;
    updateLanternSummonCopy('鱼灯化光，凝成你的鱼灯', 'THE LANTERN DISSOLVES INTO LIGHT');
    const summonBtn = document.getElementById('lantern-summon-btn');
    if (summonBtn) {
        summonBtn.disabled = true;
        summonBtn.querySelector('span').textContent = '凝成中';
        summonBtn.querySelector('small').textContent = 'DISSOLVING';
    }
    particleScene?.setDissolveManual(0.02);
}
...
function enterCraftVideoExperience(forceOpen = false) {
    ...
    if (lanternSummonStage !== 'idle') {
        if (forceOpen || currentPhase === PHASES.CRAFTING) {
    ...
    if (!overlay || !frame) {
        ...
        showToast('⚠️ 制灯界面容器缺失，请检查页面结构', 2400);
        return;
    }
...
function updateLanternSummonSweep(palmX, isOpenPalm) {
    if (currentPhase === PHASES.LANTERN_RECEIVE && lanternSummonStage === 'particles' && !lanternParticleDissolveStarted) {
        lanternParticleGestureFrames = isOpenPalm
            ? lanternParticleGestureFrames + 1
            : Math.max(0, lanternParticleGestureFrames - 2);
        if (lanternParticleGestureFrames >= 10) startLanternParticleDissolve();
        return;
    }
...
function updateOkConfirmGesture(isOk) {
    ...
    if (currentPhase === PHASES.FISHING && catchShowcaseActive) {
        ...
        enterLanternDance();
    } else if (currentPhase === PHASES.LANTERN_SWARM && lanternSummonStage === 'model') {
        enterLanternDance();
    }
}

// ── 粒子场景：实体鱼灯 ⇄ 粒子鱼影 溶解互动（来自 pointcloud-demo，已模块化） ──
async function activateParticleScene(options = {}) {
    ...
    try {
        ...
        if (!particleScene) {
            particleScene = new ParticleScene(layer, {
                ...
                handDrivesDissolve: options.handDrivesDissolve ?? true,
            });
        }
...
function setupUI() {
    ...
    document.getElementById('lantern-summon-btn')?.addEventListener('click', () => {
        ...
        else if (lanternSummonStage === 'particles') startLanternParticleDissolve();
...
function animate() {
    ...
    if (particleSceneActive && particleScene) {
        ...
        if (currentPhase === PHASES.LANTERN_RECEIVE && lanternSummonStage === 'particles' && lanternParticleDissolveStarted) {
            lanternParticleDissolveElapsed += dt;
            const progress = Math.min(1, lanternParticleDissolveElapsed / 15);
            const eased = 1 - Math.pow(1 - progress, 3);
            particleScene.setDissolve(eased);
            if (progress >= 1) {
                completeLanternParticleReceive();
                return;
            }
        }
