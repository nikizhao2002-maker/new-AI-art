...
async function setBgmTrack(mode, { restart = true } = {}) {
    ...
    try {
        ...
        audio.muted = false;
        await audio.play();
        if (token !== bgmSwitchToken) return;
    } catch (err) {
        // 等下一次用户手势或自动启动再次尝试。
    }
}
...
function updateLanternDanceAnimation(dt, time) {
    ...
    lanternDanceInstances.forEach((inst, index) => {
        ...
        const waveTime = time * (0.72 + followPower * 0.18) + inst.phase + index * 0.34;
        const palmWave = Math.sin(waveTime);
        const rippleWave = Math.cos(waveTime * 0.82 + index * 0.45);
        ...
        inst.mesh.rotation.y = Math.PI / 2 + inst.rotationOffset
            + Math.sin(time * inst.speed + inst.phase) * 0.08
            + followDir * followPower * 0.045
            + palmWave * followPower * 0.015;
        inst.mesh.rotation.z = Math.sin(time * inst.speed * 1.2 + inst.phase) * 0.025
            + rippleWave * followPower * 0.01;
    });
    }
    ...
    if (lanternDanceReleaseProgress >= 1) {
        updateLanternSummonCopy('张开手掌移动，带着鱼灯群游动', 'MOVE YOUR PALM TO GUIDE THE LANTERNS THROUGH THE WATER');
...
function createNightSparkles() {
    ...
    nightSparkles.userData = { baseColors, phases, speeds };
    scene.add(nightSparkles);
}

function spawnMiniFirework(time, variant = 'mixed', slot = null) {
...
function updateNightEmbellishments(dt, time) {
    ...
    if (nightSparkles?.material) {
        ...
        if (nightActive) {
            ...
            const { baseColors, phases, speeds } = nightSparkles.userData;
