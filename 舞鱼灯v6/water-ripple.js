...
const renderFragmentShader = `
    ...
    uniform float uTime;
    uniform float uInkStrength;     // 水墨化强度 0=原图 1=全水墨
    uniform vec3  uInkTint;         // 青灰墨色
    uniform vec3  uAccentWarm;      // 暖金点睛色
    uniform float uVignette;        // 墨边晕染强度
    uniform sampler2D uInkFlow;     // 墨韵入水：墨密度场
    ...
    void main() {
        ...
        // ── 水墨写意调色（uInkStrength 控制，由 PHASE_LOOK 驱动）──
        float lum = dot(color, vec3(0.299, 0.587, 0.114));
        vec3 inked = mix(vec3(lum), color, 0.62);                      // 减饱和 → 墨韵
        inked *= mix(vec3(1.0), uInkTint / 0.55, 0.35);                // 罩青灰
        inked += uAccentWarm * smoothstep(0.78, 0.97, lum) * 0.18;     // 高光处一抹暖金
        color = mix(color, inked, uInkStrength);

        // 墨边 vignette：纸缘吃墨，复用波动纹理扰动半径模拟晕染毛边
        vec2 vc = vUv - 0.5;
        float edge = length(vc * vec2(1.0, 1.25));
        float bleed = texture2D(uWaveData, vUv * 0.5 + 0.25).r * 0.15;
        color *= 1.0 - uVignette * smoothstep(0.46 + bleed, 0.85, edge);
...
// ═══════════════════════════════════════════════════════
// 墨韵入水：墨密度场模拟（注墨 → 随波平流 → 扩散晕开 → 缓慢消散）
// 注墨点复用 uFingers（手指触水 / 环境涟漪 / 鱼接触涟漪自动带墨）
// ═══════════════════════════════════════════════════════
const inkFragmentShader = `
    precision highp float;
    uniform sampler2D uInkPrev;
    uniform sampler2D uWaveData;
    uniform float uFingers[30];
    varying vec2 vUv;
    void main() {
        vec2 texel = vec2(1.0 / ${RIPPLE_SIZE}.0);
        // 波浪梯度 → 墨随波流
        ...
        vec2 advect = vUv - grad * 5.0;
        float ink = texture2D(uInkPrev, advect).r;
        // 扩散晕开
        float blur = (
            texture2D(uInkPrev, advect + vec2(texel.x, 0.0)).r +
            texture2D(uInkPrev, advect - vec2(texel.x, 0.0)).r +
            texture2D(uInkPrev, advect + vec2(0.0, texel.y)).r +
            texture2D(uInkPrev, advect - vec2(0.0, texel.y)).r) * 0.25;
        ink = mix(ink, blur, 0.3);
        // 注墨（与波动模拟共用手指位置）
        for (int i = 0; i < 10; i++) {
            float on = uFingers[i * 3 + 2];
            if (on > 0.5) {
                vec2 p = vec2(uFingers[i * 3], uFingers[i * 3 + 1]);
                float d = distance(vUv, p);
                ink += smoothstep(0.08, 0.0, d) * 0.85;
            }
        }
        ink *= 0.9965; // 半衰期约 3.5 秒，墨痕能留住
        if (ink < 0.004) ink = 0.0;
        gl_FragColor = vec4(min(ink, 1.4), 0.0, 0.0, 1.0);
    }
`;
...
export class WaterRipple {
    constructor(renderer, camera) {
        ...
        // 墨韵入水：墨密度场 ping-pong
        this.inkA = new THREE.WebGLRenderTarget(RIPPLE_SIZE, RIPPLE_SIZE, rtOptions);
        this.inkB = new THREE.WebGLRenderTarget(RIPPLE_SIZE, RIPPLE_SIZE, rtOptions);
        this.inkMaterial = new THREE.ShaderMaterial({
            uniforms: {
                uInkPrev: { value: this.inkA.texture },
                uWaveData: { value: this.rtB.texture },
                uFingers: { value: this.waveMaterial.uniforms.uFingers.value }, // 共享同一数组引用
            },
            vertexShader: waveVertexShader,
            fragmentShader: inkFragmentShader,
        });
        this.inkScene = new THREE.Scene();
        this.inkScene.add(new THREE.Mesh(new THREE.PlaneGeometry(2, 2), this.inkMaterial));
        ...
        this.renderMaterial = new THREE.ShaderMaterial({
            uniforms: {
                ...
                uInkTint: { value: new THREE.Color(0x8da39d) },
                uAccentWarm: { value: new THREE.Color(0xd9b25f) },
                uVignette: { value: 0.0 },
                uInkFlow: { value: null },
            },
    ...
    setWatercolorMode(enabled) {
        ...
        // 水彩模式保留 0.25 焦散高光：波峰残留淡淡墨线，再由 uAccentWarm 染暖
        this.renderMaterial.uniforms.uHighlightStrength.value = enabled ? 0.25 : 1.0;
    ...
    /**
     * 水墨写意调色（由 app.js PHASE_LOOK 每帧驱动）
     * @param {number} strength 0=原图 1=全水墨
     * @param {THREE.Color} inkTint 青灰墨色
     * @param {THREE.Color} accentWarm 暖金点睛色
     * @param {number} vignette 墨边晕染强度
     */
    setInkGrade(strength, inkTint, accentWarm, vignette) {
        const u = this.renderMaterial.uniforms;
        u.uInkStrength.value = strength;
        u.uInkTint.value.copy(inkTint);
        u.uAccentWarm.value.copy(accentWarm);
    ...
    update(time) {
        ...
        // （墨韵入水已按需求停用——模拟与混合代码保留在 inkMaterial/inkFragmentShader，
        //   需要时在此恢复 ink pass 并在 renderFragmentShader 重新接入 uInkFlow）
