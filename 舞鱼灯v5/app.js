...
function resolveCraftVideoUrl() {
    ...
    return new URL('./outputs-video/index.html?v=20260612-v42-solid-craft', location.href).href;
}
...
function cloneLanternForDance(template) {
    ...
    const swim = applySwimDeformation(clone, {
        speed: 1.7, amplitude: 0.05, frequency: 1.1, tailBias: 2.3,
    });
    clone.userData._swimUniforms = swim.uniforms;
    // 灯影流彩（v4.1）：纱面色相沿身体缓慢流动，像烛光透过彩纱
    const flowU = { uFlowTime: { value: 0 }, uFlowStrength: { value: 0.2 } }; // ±11.5°：流彩可感知但不出金调
    clone.traverse((obj) => {
        if (!obj.isMesh || !obj.material) return;
        const mats = Array.isArray(obj.material) ? obj.material : [obj.material];
...
function updateLanternDanceAnimation(dt, time) {
    ...
    formationState.t += dt * (0.16 + followPower * 0.05);
    formationState.modeTimer += dt;
    if (formationState.modeTimer > 20) {
    ...
    lanternDanceInstances.forEach((inst, index) => {
        ...
        const followLag = Math.min(1, dt * (0.78 + (index % 5) * 0.1));
        ...
        const targetY = _processionVec.y
            + Math.sin(time * inst.speed * 1.3 + inst.phase) * inst.floatAmp * 0.6
    ...
    lanternDanceInstances.forEach((inst) => {
        const flicker = 0.86
            + 0.10 * Math.sin(time * 7.3 + inst.flickerPhase)
            + 0.04 * Math.sin(time * 13.1 + inst.flickerPhase * 1.7)
            + 0.03 * Math.sin(time * 2.2 + inst.flickerPhase);
        if (inst.halo) {
            inst.halo.position.copy(inst.mesh.position);
            inst.halo.material.opacity = 0.13 * flicker;
        }
        inst.emissiveMats.forEach((mat) => { mat.emissiveIntensity = 0.2 * flicker; });
        // 纱绸泳动：每盏灯相位错开 + 转弯 C 弯
        const swimU = inst.mesh.userData._swimUniforms;
        if (swimU) {
            swimU.uSwimTime.value = time + inst.flickerPhase;
            swimU.uTurnBend.value += (((inst._turn || 0) * 0.3) - swimU.uTurnBend.value) * Math.min(1, dt * 4);
        }
        // 灯影流彩：色相沿身体流动，各灯相位错开
        const flowU = inst.mesh.userData._flowUniforms;
        if (flowU) flowU.uFlowTime.value = time * 1.1 + inst.flickerPhase * 2.3;
    });
