...
export class ParticleScene {
    ...
    // 背景图 cover 适配（等价 CSS background-size: cover）
    _fitBackgroundCover() {
        ...
        if (canvasAspect > imgAspect) {
            const frac = imgAspect / canvasAspect;
            tex.repeat.set(1, frac);
            tex.offset.set(0, (1 - frac) / 2);
        } else {
            const frac = canvasAspect / imgAspect;
    ...
    async init() {
        ...
        // UnrealBloomPass 合成会破坏画布透明度（transparentBg 实际失效变黑底）。
        // 若指定了 bgUrl，把背景图改在 WebGL 内绘制（cover 适配），bloom 照常工作。
        if (this.opts.bgUrl) {
            new THREE.TextureLoader().load(this.opts.bgUrl, (tex) => {
                tex.colorSpace = THREE.SRGBColorSpace;
                this._bgTexture = tex;
                this._fitBackgroundCover();
                if (this.scene) this.scene.background = tex;
            });
        } else if (this.opts.bgStyle === 'craft') {
            // 工艺页同款墨绿夜底（程序化生成，与制灯 iframe 风格连续）
            const c = document.createElement('canvas');
            c.width = 512;
            c.height = 288;
            const ctx = c.getContext('2d');
            const base = ctx.createLinearGradient(0, 0, 512, 288);
            base.addColorStop(0, '#0a0f0f');
            base.addColorStop(0.7, '#050809');
            base.addColorStop(1, '#040708');
            ctx.fillStyle = base;
            ctx.fillRect(0, 0, 512, 288);
            const greenGlow = ctx.createRadialGradient(368, 127, 0, 368, 127, 230);
            greenGlow.addColorStop(0, 'rgba(41, 63, 55, 0.5)');
            greenGlow.addColorStop(1, 'rgba(41, 63, 55, 0)');
            ctx.fillStyle = greenGlow;
            ctx.fillRect(0, 0, 512, 288);
            const warmGlow = ctx.createRadialGradient(77, 245, 0, 77, 245, 150);
            warmGlow.addColorStop(0, 'rgba(122, 75, 38, 0.2)');
            warmGlow.addColorStop(1, 'rgba(122, 75, 38, 0)');
        ...
        this.renderer.toneMappingExposure = this.opts.exposure ?? 1.8;
        ...
        // lightScale：整体灯光缩放。领灯化光页用 ~0.45 呈现「未点亮」的素灯
        // （点亮仪式发生在 OK 收灯之后的 model 阶段，这里只是夜色中的灯坯）
        const ls = this.opts.lightScale ?? 1.0;
        const ambient = new THREE.AmbientLight(0xffffff, 2.5 * ls); this.scene.add(ambient);
        const d1 = new THREE.DirectionalLight(0xfff0dd, 2.5 * ls); d1.position.set(5, 10, 7); this.scene.add(d1);
        const d2 = new THREE.DirectionalLight(0xffe8cc, 2.0 * ls); d2.position.set(-5, 5, -3); this.scene.add(d2);
        const rim = new THREE.DirectionalLight(0xaaccff, 1.5 * ls); rim.position.set(-3, -5, -5); this.scene.add(rim);
        const fill = new THREE.DirectionalLight(0xffffff, 1.2 * ls); fill.position.set(0, -5, 10); this.scene.add(fill);
        const top = new THREE.DirectionalLight(0xffffff, 1.0 * ls); top.position.set(0, 10, 0); this.scene.add(top);
